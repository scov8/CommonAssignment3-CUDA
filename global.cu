...
__global__ void countingSortKernel1(DATATYPE *A, DATATYPE *C, int N)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    for (int i = index; i < N; i += stride)
        atomicAdd(&C[A[i]], 1); //https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#atomic-functions
}
...
void countingSortOnDevice(DATATYPE *initArray, DATATYPE *outputArray, int maxValue, int length, int THREADxBLOCK)
{
    ...
    countingSortKernel1<<<gridSize, blockSize>>>(device_initArray, count, length);
...
int main(int argc, char **argv)
{
    ...
    countingSortOnHost(initArray, host_outputArray, MAX_VALUE, length);
    ...
    for (int i = 0; i < length; i++)
    {
        if (host_outputArray[i] != device_outputArray[i])
        {
            printf("failing: H[%d]=%d is not D[%d]=%d\n", i, host_outputArray[i], i, device_outputArray[i]);
            numOfError++;
        }
    }
